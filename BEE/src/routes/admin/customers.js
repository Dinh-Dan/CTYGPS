...
function pickPayload(body, { isUpdate = false } = {}) {
  ...
  // Khi tao moi: code do BE tu sinh (KH<NNNN>), bo qua body.code de tranh nhap tay.
  // Khi update: cho phep sua code neu admin chu dong gui len.
  if (isUpdate && body.code !== undefined) set('code', String(body.code).trim());
...
router.post('/', async (req, res, next) => {
  try {
    ...
    // Tu sinh ma KH<NNNN>, retry tren ER_DUP_ENTRY de chong race.
    let insertId;
    let lastErr;
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      data.code = 'KH' + String(n).padStart(4, '0');
    ...
    if (!insertId) throw lastErr || new Error('Khong sinh duoc ma khach');

    const [rows] = await db.query(`SELECT * FROM customers WHERE id = ?`, [insertId]);
