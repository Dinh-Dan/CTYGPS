    ...
    const [rows] = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM customers
           WHERE seen_at IS NULL AND is_deleted = 0
             AND created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)) AS customers,
        ...
        (SELECT COUNT(*) FROM staff_receipts
           WHERE reviewed = 0 AND is_deleted = 0 AND status = 'active') AS staff_receipts
    `);
    res.json(rows[0]);
  } catch (err) { next(err); }
});
...
router.post('/mark-all-seen', async (req, res, next) => {
  try {
    ...
    if (!table) return res.status(400).json({ error: 'module không hợp lệ' });
    await db.query(
  ...
  try {
    const [rows] = await db.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM staff_receipts
           WHERE reviewed = 0 AND is_deleted = 0 AND status = 'active') AS receipts_unreviewed,
        (SELECT COALESCE(SUM(amount), 0) FROM staff_receipts
           WHERE reviewed = 0 AND is_deleted = 0 AND status = 'active') AS receipts_amount
    `);
    res.json(rows[0]);
  } catch (err) { next(err); }
});
